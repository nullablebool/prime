...
namespace System
{
    ...
    [MongoDB.Bson.Serialization.Attributes.BsonSerializer(typeof(BsonMoneySerializer))]
                          ...
                          IComparable 
    {
        ...
        public static IReadOnlyList<IMoneyFormatter> Formatters = XC.I.Formatters.OfType<IMoneyFormatter>().ToList();

        public String ToString(IHasContext context)
        {
            var tmpThis = this;
            var mf = Formatters.Select(x => x.FormatMoney(context, tmpThis)).FirstOrDefault(x=>x!=null);
            return mf ?? ToDecimalValue().ToString("C", (IFormatProvider)_currency ?? context.Domain.CultureInfo);
        }

        public String ToString(string format, IHasContext context)
        {
            return ToDecimalValue().ToString(format, (IFormatProvider)_currency ?? context.Domain.CultureInfo);
        }

        public override String ToString()
        {
            return ToDecimalValue().ToString("C", (IFormatProvider)_currency ?? NumberFormatInfo.CurrentInfo);
        }

        public String ToString(string format)
        {
            return ToDecimalValue().ToString(format, (IFormatProvider)_currency ?? NumberFormatInfo.CurrentInfo);
        }
        ...
        public static Money Random(double low, double high, Currency currency)
        {
            return new Money((decimal)Rnd.I.Next(low, high), currency);
        }
